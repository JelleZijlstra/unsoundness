...
That's interesting. These are contradictory conclusions. We conclude that 3 and 4 are always appropriate. So that must mean that 2 is simply too restrictive: it doesn't allow *any* sound definition of a covariant base type that offers `__OP__`, with concrete types that expand it with `__iOP__`. It will have got to go. And we can only get rid of 2 if we also get rid of 6. If we do both, we end up with a system that allows both 3 and 4, without ignores, and without the unsoundness that this whole thread is about.
...
However, in the current state of things, the unfortunate reality is that this unsoundness exists, and any potential solutions would require careful discussion to ensure that we do not break any other interfaces (e.g. primitives' `<OP>=`).
This docstring is not the place for such discussion. A tentative proposal was made in the previously linked PR thread, but it will have to be taken to the Python discussion forum before any action can be taken.
"""
